 ...
 *  test command - forge test --fork-url https://anvil.gearbox.foundation/forks/8cda3750-f367-41bb-915f-8d375f72fb06 --match-path test/GearSparkbot.t.sol
 * 
 *  1> test_addCollateral - it opens a credit account, and sets the permissions to bot, and adds collateral to the creditAccount using bot multicall
 *  2> test_withdrawCollateral - it adds and withdraws collateral in same tx using bot multicall
 *  3> test_borrowFunds - it adds collateral and then borrows minDebt limits in same tx using bot multicall
 *  4> test_repayFunds - this can't be tested, since in one tx, borrowing and repaying funds in a creditAccount is not possible technically
 *  5> test_flashloan - it adds a collateral first, and then calls 
 *  executeTrade -> this function lends a flashloan from SparkLend protocol, then adds whole flashloan amount as collateral
 *  withdraws and swaps on 1inch for profitable trade and then repays the flashloan
 * 
 */
...
contract GearSparkbotTest is Test {
    ...
    uint256 public ETH_AMOUNT = 1000000000000000000000;
    ...
    function test_flashLoan() public {
        test_addCollateral();
        bytes memory oneinchcalldata = ""; // generate 1inch calldata
        bot.executeTrade(DAI, 1e7, CREDIT_MANAGER, creditAccount, USER, oneinchcalldata);
